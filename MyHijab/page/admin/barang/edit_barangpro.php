...
if ($loc_image != ""){
      ...
      if (in_array($extension, $cek) === TRUE){
        if ($size_image < 5044070){
          ...
          if ($row->nama_image)
            unlink("img/hijab/$row->nama_image");
          ...
          try {
      			...
      			$updatedata = array(':deskripsi' => $desk, ':harga' => $harga, ':stok' => $stok, ':nama_image' => $name_image,
        ...
        echo "</br>";
			...
			try {
				...
				$updatedata = array(':deskripsi' => $desk, ':harga' => $harga, ':stok' => $stok, ':id_barang' => $id);
				$pdo->execute($updatedata);
